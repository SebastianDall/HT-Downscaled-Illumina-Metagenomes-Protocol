...
- Place the BLT/TB1 source plate in the I-DOT and run the `files/BLT_TB1_additions.idot` file. This will add 2 µl BLT/TB1.
...
- Run the `files/TSB_addition.idot`. This will add 1 µl of TSB to each well
...
- Run the `idot_pooling.csv` file. 2 ng DNA or a maximum of 9 µl from each sample will be transfered to the PCR target plate.
